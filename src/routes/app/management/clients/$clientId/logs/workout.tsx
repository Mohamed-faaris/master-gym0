...
function WorkoutLogsRoute() {
  ...
  const weekPlanTemplates = useQuery(
  ...
  const handleCopyWeekPlan = async (planId: Id<'workoutWeekPlans'>) => {
    if (!clientId || !user) return
    setIsCopying(true)
    try {
      await copyWorkoutWeekPlan({
        planId,
        targetUserId: clientId as Id<'users'>,
        authorId: user._id,
      })
      ...
      console.error('Failed to copy week plan template', error)
    } finally {
      setIsCopying(false)
    }
  }
  ...
  return (
    <div className="space-y-6 p-4 pb-20 max-w-4xl mx-auto">
      ...
      <h2 className="text-xl font-bold mb-4">Client Workout Plans</h2>
      ...
      <div className="space-y-4 mb-8">
              ...
              <Card key={routine._id}>
                ...
                <CardContent>
                  ...
                  <div className="flex items-center gap-2 mt-4">
                    ...
                    <Link
                      to="/app/management/clients/$clientId/routines/$routineId"
                      params={{ clientId, routineId: routine._id }}
                      className="flex-1"
                    >
        ...
        {weekPlans && weekPlans.length > 0 ? (
          ...
          <Card>
            <CardContent className="pt-6 text-center text-muted-foreground">
              This client does not have any assigned week plans.
            </CardContent>
          </Card>
        )}
      </div>
              ...
              <CardContent className="pt-6">
                <div className="space-y-3">
                  <div className="flex items-start justify-between">
                    <div>
                      <p className="font-semibold text-lg">{session.status} session</p>
      ...
      <Drawer open={isTemplateDrawerOpen} onOpenChange={setIsTemplateDrawerOpen}>
        <DrawerContent>
          <div className="mx-auto w-full max-w-lg">
            <DrawerHeader>
              <DrawerTitle>Assign Workout Template</DrawerTitle>
              <DrawerDescription>
                Copy routine or week plan template to this client profile.
              </DrawerDescription>
            </DrawerHeader>
            <div className="p-4 pb-0 space-y-5 max-h-[60vh] overflow-y-auto">
              <div className="space-y-3">
                <p className="text-sm font-semibold">Routine Templates</p>
              ...
              <div className="space-y-3">
                <p className="text-sm font-semibold">Week Plan Templates</p>
                {weekPlanTemplates && weekPlanTemplates.length > 0 ? (
                  weekPlanTemplates.map((plan) => (
                    <Card
                      key={plan._id}
                      className="cursor-pointer hover:border-primary transition-colors"
                      onClick={() => handleCopyWeekPlan(plan._id)}
                    >
                      <CardHeader className="p-4">
                        <CardTitle className="text-base flex items-center justify-between">
                          {plan.name}
